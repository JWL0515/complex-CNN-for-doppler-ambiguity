...
class CONFIG:
    seed = 515
    model_name = 'multi classes + cross_entropy'
...
def train(model, device, train_loader, optimizer, loss_func, epoch, loss_dict, acc_dict):
    ...
    acc_dict['Epoch'].append(epoch + 1)
    acc_dict['train_accuracy'].append(accuracy_score(predict['targets'], predict['predictions']))
    print(f"Training loss: {train_loss}")


def valid(model, device, valid_loader, loss_func, epoch, loss_dict, acc_dict):
    predict = {'targets': [], 'predictions': []}
    model.eval()
    valid_losses = []
    avg_valid_losses = []
    ...
    acc_dict['valid_accuracy'].append(accuracy_score(predict['targets'], predict['predictions']))
    print(f"Valid loss: {valid_loss}")

    return valid_loss


def test(model, device, loss_func, test_loader, model_pth=None):
...
class ComplexNet(nn.Module):
    ...
    def __init__(self):
        ...
        self.conv1 = ComplexConv2d(1, 50, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1))
...
if __name__ == "__main__":
    ...
    # main_path = '../input/mixed-400records/Dataset Quan16b/Dataset Quan16b'
    # test_path = '../input/mixed-400records/Dataset Quan16b/Dataset Quan16b'

    # dataset = PrepareDataset(main_path, True)
    # test_data = PrepareDataset(test_path, True)
    ...
    # ten_in = torch.randn(4, 1, 50, 64, dtype=torch.cfloat)
    ...
    #     valid_loss = valid(model, CONFIG.device, valid_loader, CONFIG.loss_func, epoch, loss_dict, acc_dict)
    #     scheduler.step()
    ...
    test_path = 'rxNF=51_decimated_quan_normalized'
    test_data = PrepareDataset(test_path)
    test_loader = DataLoader(test_data, batch_size=CONFIG.batch_size, shuffle=False)

    model.eval()
    model.load_state_dict(torch.load('1_target_models/50x64/conv2D/f1a/best_model_test_acc.pt'))
    ...
    predictions = pd.concat([test_data.df.record_name, test_data.df.frame_name, test_data.df.t1_true_velocity,test_data.df.t1_classID,predict], axis=1)
    predictions.to_csv(f'rxNF=51_decimated_quan_normalized/prediction_with_test_model.csv', index=False)
    print(accuracy_score(predict['targets'], predict['predictions']))
    model.eval()
    model.load_state_dict(torch.load('1_target_models/50x64/conv2D/f1a/best_model_valid_acc.pt'))
    predict = {'targets': [], 'predictions': [], 'KCNN_times': []}
    KCNN_time_l = []
    KCNN_time = 0
    with torch.no_grad():
    ...
    predict = pd.DataFrame(predict)
    predictions = pd.concat([test_data.df.record_name, test_data.df.frame_name, test_data.df.t1_true_velocity,test_data.df.t1_classID,predict], axis=1)
    predictions.to_csv(f'rxNF=51_decimated_quan_normalized/prediction_with_valid_model.csv', index=False)
    print(accuracy_score(predict['targets'], predict['predictions']))
